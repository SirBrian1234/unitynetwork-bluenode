...
public class SocketFunctions {
    ...
    public static InetAddress getAddress(String PhAddress) throws UnknownHostException {
        ...
        try {
            IPaddress = InetAddress.getByName(PhAddress);
        	 ...
        	 App.bn.ConsolePrint(pre + "UNKNOWN HOST "+PhAddress);
        	 throw ex;
        }
        return IPaddress;
    }
    ...
    public static Socket absoluteConnect(InetAddress IPaddress, int authPort) throws Exception {
        ...
        try {
            ...
            App.bn.ConsolePrint(pre + "CONNECTION REFUSED FOR "+IPaddress.getHostAddress()+" "+authPort);
            throw ex;
        } catch (java.net.SocketTimeoutException ex) {
            App.bn.ConsolePrint(pre + "CONNECTION TIMED OUT FOR "+IPaddress.getHostAddress()+" "+authPort);
            throw ex;
        } catch (IOException ex) {
            App.bn.ConsolePrint(pre + "CONNECTION ERROR FOR "+IPaddress.getHostAddress()+" "+authPort);
            throw ex;
        }
        return socket;
    }
    ...
    public static byte[] receiveData(DataInputStream reader) throws IOException {
		...
		if (read > 0) {
			...
			if (byteT[0] == (int)0) {
				App.bn.ConsolePrint(pre + "RECEIVED a zero char");
		    } else if (byteT[0] == (int)13) {
				App.bn.ConsolePrint(pre + "RECEIVED a new line char");
			} else if (byteT[0] == (int)10) {
				App.bn.ConsolePrint(pre+ "received a return char");
			}
		} else if (read == 0){
    ...
    public static void sendPlainStringData(String message, DataOutputStream writer) throws Exception {
    	if (message == null) {
        	App.bn.ConsolePrint(pre + "NO DATA TO SEND");
