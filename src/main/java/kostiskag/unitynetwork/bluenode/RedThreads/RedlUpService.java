...
public class RedlUpService extends Thread {
    ...
    public RedlUpService(String vaddress) {
        ...
        sourcePort = App.UDPports.requestPort();
        pre = pre + vaddress+" ";
    }

    @Override
    public void run() {
        App.ConsolePrint(pre + "STARTED FOR " + vaddress + " AT " + Thread.currentThread().getName() + " ON PORT " + sourcePort);        
        
        try {
            serverSocket = new DatagramSocket(sourcePort);
        } catch (java.net.BindException ex) {
            App.ConsolePrint(pre + "PORT ALLREADY BINDED, EXITING");
            return;
        ...
        while (!kill) {
            data = null;
            try {
                data = App.localRedNodesTable.getRedNodeInstanceByAddr(vaddress).getQueueMan().poll();
                ...
                if (version.equals("0")) {
                    ...
                    if (args.length > 1) {
                        if (args[0].equals("00000")) {
                            ...
                            App.TrafficPrint(pre + "LE WILD RN DPING LEAVES", 1, 0);
                        }
                    } else {
                        System.out.println(pre + "wrong length");
                    }
    ...
    public void kill() {
        kill = true;
        serverSocket.close();        
        App.localRedNodesTable.getRedNodeInstanceByAddr(vaddress).getQueueMan().clear();
    }
